...
match* match_descriptors(descriptor* a, int an, descriptor* b, int bn,
  ...
  *mn = an;
  ...
  int count = 0;
  int* seen = (int*)calloc(bn, sizeof(int));
...
image combine_images(image a, image b, matrix H) {
  ...
  botright.x = std::max(c1.x, std::max(c2.x, std::max(c3.x, c4.x)));
  botright.y = std::max(c1.y, std::max(c2.y, std::max(c3.y, c4.y)));
  topleft.x = std::min(c1.x, std::min(c2.x, std::min(c3.x, c4.x)));
  topleft.y = std::min(c1.y, std::min(c2.y, std::min(c3.y, c4.y)));

  // Find how big our new image should be and the offsets from image a.
  int dx = std::min((float)0, topleft.x);
  int dy = std::min((float)0, topleft.y);
  int w = std::max((float)a.w, botright.x) - dx;
  int h = std::max((float)a.h, botright.y) - dy;
