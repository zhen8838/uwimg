...
matrix copy_matrix(matrix m) {
  ...
  for (i = 0; i < m.rows; ++i) {
    for (j = 0; j < m.cols; ++j) { c.data[i][j] = m.data[i][j]; }
  }
  return c;
}
...
matrix augment_matrix(matrix m) {
  ...
  for (j = 0; j < m.rows; ++j) { c.data[j][j + m.cols] = 1; }
  return c;
}
...
matrix make_identity(int rows, int cols) {
  ...
  for (i = 0; i < rows && i < cols; ++i) { m.data[i][i] = 1; }
  return m;
}
...
matrix matrix_elmult_matrix(matrix a, matrix b) {
  ...
  for (i = 0; i < p.rows; ++i) {
    for (j = 0; j < p.cols; ++j) { p.data[i][j] = a.data[i][j] * b.data[i][j]; }
  }
  return p;
}
...
matrix transpose_matrix(matrix m) {
  ...
  t.rows = m.cols;
  t.cols = m.rows;
  t.data = (double**)calloc(t.rows, sizeof(double*));
  t.shallow = 0;
  int i, j;
  for (i = 0; i < t.rows; ++i) {
    t.data[i] = (double*)calloc(t.cols, sizeof(double));
    for (j = 0; j < t.cols; ++j) { t.data[i][j] = m.data[j][i]; }
  }
  return t;
}
...
void scale_matrix(matrix m, double s) {
  int i, j;
  for (i = 0; i < m.rows; ++i) {
    for (j = 0; j < m.cols; ++j) { m.data[i][j] *= s; }
  }
}

double* matrix_mult_vector(matrix m, double* v) {
  double* p = (double*)calloc(m.rows, sizeof(double));
  int i, j;
  for (i = 0; i < m.rows; ++i) {
    for (j = 0; j < m.cols; ++j) { p[i] += m.data[i][j] * v[j]; }
  }
  return p;
}
...
void print_matrix(matrix m) {
  ...
  for (i = 0; i < m.rows; ++i) {
    printf("|  ");
    for (j = 0; j < m.cols; ++j) { printf("%15.7f ", m.data[i][j]); }
    printf(" |\n");
  }
  printf("|__");
  for (j = 0; j < 16 * m.cols - 1; ++j) printf(" ");
...
double* LUP_solve(matrix L, matrix U, int* p, double* b) {
  int i, j;
  double* c = (double*)calloc(L.rows, sizeof(double));
  for (i = 0; i < L.rows; ++i) {
    int pi = p[i];
    c[i] = b[pi];
    for (j = 0; j < i; ++j) { c[i] -= L.data[i][j] * c[j]; }
  }
  for (i = U.rows - 1; i >= 0; --i) {
    for (j = i + 1; j < U.cols; ++j) { c[i] -= U.data[i][j] * c[j]; }
    c[i] /= U.data[i][i];
  }
  return c;
}
...
matrix matrix_invert(matrix m) {
  ...
  for (k = 0; k < c.rows; ++k) {
    ...
    c.data[k][k] = 1;
    for (j = k + 1; j < c.cols; ++j) { c.data[k][j] /= val; }
    for (i = k + 1; i < c.rows; ++i) {
      double s = -c.data[i][k];
      c.data[i][k] = 0;
      for (j = k + 1; j < c.cols; ++j) { c.data[i][j] += s * c.data[k][j]; }
    }
  }
  ...
  for (i = 0; i < m.rows; ++i) {
    for (j = 0; j < m.cols; ++j) { inv.data[i][j] = c.data[i][j + m.cols]; }
  }
  free_matrix(c);
  // print_matrix(inv);
  return inv;
...
int* in_place_LUP(matrix m) {
  int* pivot = (int*)calloc(m.rows, sizeof(int));
  if (m.rows != m.cols) {
    fprintf(stderr, "Matrix not square\n");
    return 0;
  }
...
double mag_matrix(matrix m) {
  ...
  double sum = 0;
  for (i = 0; i < m.rows; ++i) {
    for (j = 0; j < m.cols; ++j) { sum += m.data[i][j] * m.data[i][j]; }
  }
  return sqrt(sum);
}
...
matrix load_matrix(const char* fname) {
  ...
  int i;
  matrix m = make_matrix(rows, cols);
  for (i = 0; i < rows; ++i) { fread(m.data[i], sizeof(double), cols, fp); }
  fclose(fp);
  return m;
}
